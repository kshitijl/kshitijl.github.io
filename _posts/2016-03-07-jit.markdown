...
their own projects. PyPy is also based on JIT
compilation. It's everywhere now!
...
Anyway, looks like we just need the 1 byte opcode `retq` and its
...
single integer return value is just put in the register `%rax`, which
means that if a function simply returns, it's returning whatever was
in `%rax`.
...
looking at the disassembly of call to `func()`
...
or the sequence of bytes `0xb8 0x05 0x00 0x00` to return 5.
