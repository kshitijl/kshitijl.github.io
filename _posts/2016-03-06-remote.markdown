...
presents me with a touchpad and a link to a keyboard. Here's the
traffic:
...
will be easier to understand. `tcpdump` tells us the length of each
UDP packet is 16, so I kill the listener and start again:
...
order, so next I type `abcddcba` at the app and get this output on
`netcat`:
...
`a`. The last four bytes are some kind of increasing sequence. It's
...
I to browse the Netflix website on my laptop, select what I want to
...
movie I want to watch, play it on my laptop, and copy the URL. Now I
go to the `ssh` session:

    google-chrome <video URL>

Too bad it's not this easy. Chrome tries to start on my laptop instead
...
does the trick. All that remains is to add another keyboard shortcut,
`Ctrl-W` to the server for closing a tab when I'm done with one video
and want to start another. Otherwise Netflix complains I'm watching
from too many open tabs.
